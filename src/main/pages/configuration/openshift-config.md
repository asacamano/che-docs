...
You can configure deployment of Che on OpenShift with env variables that are defined in `che-config` file. You can add new envs or edit existing ones. After the file is saved, run the deployment script with `rollupdate` flag. Che deployment will be updated which automatically triggers a new deployment. You can also export envs in your environment, and the script will pick them up.
